...
$update = $conn->prepare('UPDATE accounts SET status=1 WHERE id=?');
$update->bind_param('i', $id);
if ($update->execute()) {
    // Log to activity_log_admin
    $action = 'activate_user';
    $details = "Activated user (ID: $id)";
    ...
    $message = 'Your account has been reactivated by an administrator. You may now log in and use the system.';
